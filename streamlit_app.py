...
if "columns_ref" not in st.session_state:
    st.session_state.columns_ref = None  # reference format from upload

# ----------------------------
# Step 0: Upload Reference Dataset
# ----------------------------
uploaded_file = st.file_uploader(
    "Upload reference dataset (CSV/TSV) for format:",
    type=["csv", "tsv"]
)
...
if uploaded_file is not None:
    try:
        ...
        df_ref = pd.read_csv(uploaded_file, sep="\t")

    if "RequirementText" not in df_ref.columns or "NFR" not in df_ref.columns:
        st.error("Reference file must contain at least 'RequirementText' and 'NFR' columns.")
    else:
        st.session_state.columns_ref = list(df_ref.columns)
        st.success(f"Reference format loaded: {st.session_state.columns_ref}")
        st.dataframe(df_ref.head())

# ----------------------------
# Step 1: Gemini dataset generation (always in reference format)
# ----------------------------
if st.session_state.columns_ref is not None:
    ...
    if st.button("Generate Dataset via Gemini"):
        try:
            ...
            # Build prompt based on reference columns
            col_str = ", ".join(st.session_state.columns_ref)
            prompt = f"""
            Generate {num_rows} software requirement records in CSV format 
            with EXACTLY these columns: {col_str}.
            - Each row must have values for all columns.
            - The 'RequirementText' column should contain requirement sentences.
            - The 'NFR' column should contain the requirement type/label.
            - Do not add commas or quotes inside the fields.
            - Separate columns strictly with a comma.
            ...
            # Keep only valid rows with correct column count
            lines = raw_text.split("\n")
            csv_lines = [line for line in lines if len(line.split(",")) == len(st.session_state.columns_ref)]
            csv_text = "\n".join(csv_lines)

            df = pd.read_csv(StringIO(csv_text), names=st.session_state.columns_ref)
            df["cleaned"] = df["RequirementText"].apply(clean_text)
...
if st.session_state.df is not None:
    ...
    model_choice = st.selectbox(
        "Choose Model", ["Naive Bayes", "SVM", "Random Forest", "CNN", "LSTM"]
    )
    run_button = st.button("Run Model")
